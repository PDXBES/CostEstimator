    ...
    " CREATE INDEX IDX_CapitalCostsMobilizationRatesAndTimes_GLOBALID ON AMStudio_CapitalCostsMobilizationRatesAndTimes (GLOBALID); ";
        ...
        public static string setWaterRelocation(double relocationCostPerInchDiameter, double relocationCostBase)
        {
            return "UPDATE AMStudio_PipeDetails " +
                   "SET    [ParallelWaterRelocation] = ( " +
                     "SELECT pWtrMaxD *" + relocationCostPerInchDiameter.ToString() + "+ " + relocationCostBase.ToString() + " * 10 * pWtr " +
                     "FROM   XPData " +
        ...
        public static string setEnvironmentalMitigation(double envMitigationCost, double envMitigationWidth)
        {
            return "UPDATE AMStudio_PipeDetails " +
                   "SET    [EnvironmentalMitigation] = ( " +
                     "SELECT " + envMitigationCost.ToString() + "*((IFNULL(xFtEzonC,0) + IFNULL(xFtEzonP,0)) *" + envMitigationWidth.ToString() + ")/43560.0  " + //43560 square feet per acre
                     "FROM   XPData " +
        ...
        public static string setPipeZoneBackfill(double pipeZoneDepthAdditionalInches, double pipeZoneBackfillCost)
        {
            return "UPDATE AMStudio_PipeDetails " +
                   "SET    [PipeZoneBackfill] = ( " +
                     ...
                     "        /27.0 - (((OutsideDiameter*OutsideDiameter) * 12.0 * (3.14159/4.0))/46656.0) " +//12 inches per foot, 27 cubic feet per cubic yard, 46656 cubic inches per cubic yard
        ...
        public static string setTrenchExcavation()
        {
            return "UPDATE AMStudio_PipeDetails " +
                   "SET     [TrenchExcavation] = ( " +
                     "SELECT ExcavationVolume * costPerCuYd * Length " +
        ...
        public static string setPipeMaterialBaseCostPerFoot(string material)
        {
            return "UPDATE AMStudio_PipeDetails " +
                   "SET    [PipeMaterialBaseCostPerFoot] = IFNULL(( " +
                     ...
                     "WHERE  XPData.ID = AMStudio_PipeDetails.ID ORDER BY minDiameter DESC LIMIT 1),0); ";//AND PipeMaterialCosts.Material like '" + material + "' ORDER BY minDiameter DESC LIMIT 1),0); ";
        }
        ...
        public static string setDifficultArea(double difficultAreaFactor)
        {
            return "UPDATE AMStudio_PipeDetails " +
                   ...
                   "                    CASE WHEN IFNULL(pRail, 0) >0 OR IFNULL(xRail,0) > 0 THEN 1 ELSE 0 END + " +
                   "                    CASE WHEN IFNULL(pLRT, 0) > 0 OR IFNULL(xLRT, 0) > 0 THEN 1 ELSE 0 END + " +
        ...
        {
            double CCIFrac = currentENR / BaseENR;
            return "UPDATE AMStudio_PipeDetails " +
                   "SET    DirectConstructionCost = (IFNULL(DifficultArea, 1) * (" + CCIFrac.ToString() + ")) " +
                                                   "*" +
                                                   "(" +
                   ...
                   "                               SELECT (IFNULL(DifficultArea, 1) * (" + CCIFrac.ToString() + ")) " +
                                                   "*" +
                                                   "(" +
                                                     " IFNULL(TrenchShoring, 0) " +
                                                     "+ IFNULL(TrenchExcavation, 0 ) " +
